...
while spot detection is performed in $O(t)$, where $t$ is the number of terms in the query, disambiguation usually involves the pairwise comparison ($O(t^2)$) of the spots
detected in order to compute the \emph{relatedness} distance among the candidate entities.
...
The linker exploits a dictionary containing more than 10 million mentions extracted from titles and anchor texts of a recent English dump 
of Wikipedia. Given a query, all the possible n-grams are generated and matched against the dictionary. We annotated $13,977$ ($70\%$)
queries in \head{} with at least one spot, and respectively $4,901,987$ ($63\%$) queries in \tail{}. For each spot we also collected: 
i) the \textbf{position in the query}, the start and end position in the query ii) the \textbf{link probability (lp)} the probability
for the spot to link to an entity, computed as the number of occurrences of the spot as anchor text divided by the number of occurrences
as simple text and iii) \textbf{the candidate entities} a list of possible candidate entities for the spot, and for each candidate
its \textbf{commonness}, the probability $p(e|s)$ the spot $s$ refers to the entity $e$ computed considering how many times on Wikipedia
